...
public class AccuracyLocationFilter: LocationFilter {
    ...
    public func shouldUpload(currentLocation: LocationEntity, previousLocation: LocationEntity?, trackerConfig: LocationTrackerConfig) -> Bool {
        if (previousLocation == nil || previousLocation?.id == nil) { return true }
        ...
        let shouldUpload = distanceInMeters > currentLocation.accuracy
        return shouldUpload
    }
}
